...
class SendMailEtalonRefusCommand extends GlobalCommand {

    protected function configure() {
        $this->sNomCommande = 'sendmail_etalon_refus';
        $this->setName($this->sNomCommande);
        $this->setDescription("Envoi par e-mail d'un refus d'étalonnage.")
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $this->oLog->info('Lancement de l\'envoi de refus d\'étalonnage via ' . $this->sNomCommande);
...
/*            $sUrlBase = $this->container->getParameter('MROAD_VERSION_'.  strtoupper($this->container->get('kernel')->getEnvironment()).'_URL');
            ...
            $_etalon = $em->getRepository('AmsModeleBundle:Etalon')->selectOne($_etalon_id," and date_demande is not null and date_refus is not null");
//            $_etalon_tournee = $em->getRepository('AmsModeleBundle:EtalonTournee')->select($_etalon_id);
            
            if (!empty($_etalon)) {
                $sTemplate = 'AmsModeleBundle:Emails:mail_etalon_refus.mail.twig';
                $aMailDatas = array(
                    'sSubject' => 'Refus d\'étalonnage',
...
//                    'etalon_tournee' => $_etalon_tournee
                ...
                $sTemplate = 'AmsModeleBundle:Emails:mailerreur_etalon_refus.mail.twig';
                $aMailDatas = array(
                    'sSubject' => 'ERREUR : Refus d\'étalonnage',
