...
#*-*-*-*-*- Export des comptes rendus de distribution vers DAN JADE
echo '<h3><u>Export des comptes rendus de distribution vers DAN JADE</u></h3>'
echo '<pre>'
php ./app/console cr_distribution_export JADE_EXP_CR_DISTRIBUTION J+0 J+0 --pqr_id=39 --jn=nuit --soc=tout --id_sh=$id_sh --id_ai=$id_ai --env=$env > $fic_log 2>&1
cat $fic_log
echo '</pre>'
echo '<br /><br />'
