...
class PaiDiffuseurRepository extends GlobalRepository {
    ...
    function selectTournee($depot_id, $flux_id, $date_distrib, $sqlCondition='') {
        $sql = "SELECT DISTINCT
                ...
                FROM pai_tournee pt
                LEFT OUTER JOIN pai_prd_tournee ppt ON ppt.tournee_id=pt.id AND ppt.natureclient_id=1 -- Diffuseur
    ...
    function selectProduit($depot_id, $flux_id, $date_distrib) {
        $sql = "SELECT DISTINCT
                ...
                WHERE pt.depot_id=" . $depot_id . " AND pt.flux_id=" . $flux_id . " AND pt.date_distrib='" . $date_distrib . "'
                AND ppt.natureclient_id=1 -- Diffuseur
        ...
        try {
            foreach ($param as $key => $value) {
                if (substr($key, 0, 1) == "Q") {
                    ...
                    $nbRows = $this->_em->getRepository("AmsPaieBundle:PaiPrdTournee")->update($user, $param["gr_id"], $produit_id, 1, $qte, $nbcli);
                    if ($nbRows == 0) {
                        $this->_em->getRepository("AmsPaieBundle:PaiPrdTournee")->insert($user, $param["gr_id"], $produit_id, 1, $qte, $nbcli);
                    } else {
                        $this->_em->getRepository("AmsPaieBundle:PaiPrdTournee")->nettoyage($param["gr_id"], $produit_id, 1);
                    }
                    $this->_em->getRepository("AmsPaieBundle:PaiPrdTournee")->recalcul_tournee_produit_nature($param["gr_id"], $produit_id, 1);
                }
            }
