 ...
 * 
 * Exemple de commande : 
 *                      php app/console cas_histo J-150 J-120 --id_sh=cron_test --id_ai=1  --env=dev
 * 
 * 
 * @author aandrianiaina
 *
 */
class ClientAServirHistoCommand extends GlobalCommand
{
    protected function configure()
    {
    	$this->sNomCommande	= 'cas_histo';
        $sJourATraiterMinParDefaut = "J-150";
        $sJourATraiterMaxParDefaut = "J-120";
    	$this->setName($this->sNomCommande);
    	// Pour executer, faire : php app/console cas_histo Expl : php app/console cas_histo J+1 J+3 --env=prod
        $this
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->endTraitement();   
        if($input->getOption('id_ai') && $input->getOption('id_sh')){
            $this->oLog->info(date("d/m/Y H:i:s : ")."Fin Historisation automatique des tables client_a_servir_logist & client_a_servir_src - Commande : ".$this->sNomCommande." ".$sJourATraiterMin." ".$sJourATraiterMax." --id_sh=".$idSh." --id_ai=".$idAi."  --env=".$sEnvironnement." ");   
        }else{
