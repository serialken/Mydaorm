...
/*
  truncate table int_mroad;
      insert into int_mroad(rcoid)
      select "oid"  from "pai_png_relationcontrat"@MROAD
        ;
      commit;
      declare
            ret integer;
    begin
      ret:=DBMS_HS_PASSTHROUGH.EXECUTE_IMMEDIATE@MROAD('delete from pai_png_anciennetectr');
...
CREATE DATABASE LINK MROAD CONNECT TO "mroad" identified by "Mroad123" using 'PMROAD';
...
*/
    ...
    begin
      ...
      ret:=DBMS_HS_PASSTHROUGH.EXECUTE_IMMEDIATE@MROAD('delete from pai_png_horaire');
      ret:=DBMS_HS_PASSTHROUGH.EXECUTE_IMMEDIATE@MROAD('delete from pai_png_semainetype');
    ...
    begin
      ...
      select distinct rc.oid
        ...
        where exists (select null from rcpopulationw rcw
                    ...
                    where rcw.relationcontrat=rc.oid and rc.relatdatefinw>=to_date('20140821','YYYYMMDD')
...
*/
     ...
     for ta in (
      ...
      for f in (
        ...
        from  legalferie f
        where f.legalferiedate>=to_date('20140821','YYYYMMDD')
        ) loop
...
--        and   end_date      >=to_date('20140821','YYYYMMDD')
        ) loop
...
--        and   end_date      >=to_date('20140821','YYYYMMDD')
    ...
    procedure exec_anciennete is
    begin
      for a in (
        select distinct a.oid,a.timestamp,a.ancctrmatricule,a.begin_date,a.end_date,a.ancctrgroupe,a.ancctrsociete,a.ancctrpaie,a.ancctrmaladiew
        from  int_mroad r,
              contrat c,
              anciennetectr a
        where r.rcoid           =c.ctrrelation
        and   c.oid          =a.ancctrmatricule
--        and   end_date      >=to_date('20140821','YYYYMMDD')
        ) loop
        insert into "pai_png_anciennetectr"@MROAD("oid","timestamp","ancctrmatricule","begin_date","end_date","ancctrgroupe","ancctrsociete","ancctrpaie","ancctrmaladiew") values(a.oid,a.timestamp,a.ancctrmatricule,a.begin_date,a.end_date,a.ancctrgroupe,a.ancctrsociete,a.ancctrpaie,a.ancctrmaladiew);
      end loop;
      commit;
    end;
    ...
    procedure exec_horaire is
    begin
      for s in (
        select s.oid,s.timestamp,s.semtypecode,s.semtypelibelle,to_char(s.semtypedurlun,'99.99') as semtypedurlun,to_char(s.semtypedurmar,'99.99') as semtypedurmar,to_char(s.semtypedurmer,'99.99') as semtypedurmer,to_char(s.semtypedurjeu,'99.99') as semtypedurjeu,to_char(s.semtypedurven,'99.99') as semtypedurven,to_char(s.semtypedursam,'99.99') as semtypedursam,to_char(s.semtypedurdim,'99.99') as semtypedurdim
        from semainetype s
        ) loop
        insert into "pai_png_semainetype"@MROAD("oid","timestamp","semtypecode","semtypelibelle","semtypedurlun","semtypedurmar","semtypedurmer","semtypedurjeu","semtypedurven","semtypedursam","semtypedurdim") 
        values(s.oid,s.timestamp,s.semtypecode,s.semtypelibelle,s.semtypedurlun,s.semtypedurmar,s.semtypedurmer,s.semtypedurjeu,s.semtypedurven,s.semtypedursam,s.semtypedurdim);
      end loop;

      for h in (
        select distinct h.oid,h.timestamp,h.horcontr,h.begin_date,h.end_date,h.horsemainetype
        from int_mroad r,
            horaire h
        where r.rcoid             =h.horcontr
        ) loop
        insert into "pai_png_horaire"@MROAD("oid","timestamp","horcontr","begin_date","end_date","horsemainetype") 
        values(h.oid,h.timestamp,h.horcontr,h.begin_date,h.end_date,h.horsemainetype);
      end loop;
      commit;
    end;
