...
php app/console import_fic_ftp SUIVI_PRODUCTION --id_sh=$id_sh --id_ai=$id_ai --env=$env > $fic_log 2>&1
#cat $fic_log


#*-*-*-*-*- Integration en BDD des fichiers de production
echo "Insertion/MAJ des fichiers en BDD ... "
php app/console suivi_de_production_integration  --id_sh=$id_sh --id_ai=$id_ai --env=$env > $fic_log 2>&1
#cat $fic_log
