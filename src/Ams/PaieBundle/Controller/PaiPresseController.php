...
class PaiPresseController extends GlobalPaiController {

    public function getRepositoryName() { return $this->getBundleName().':PaiPresse'; }
    public function getServiceName() { return 'ams.repository.paipresse'; }
    public function getRoute() { return 'liste_pai_presse'; }
    ...
    public function listeAction(Request $request) {
        ...
        return $this->render($this->getTwigListe(), array(
            ...
            'comboProduitAdd' => $this->getComboHtml($em->getRepository('AmsPaieBundle:PaiPrdTournee')->selectComboAjouterPresse($session->get("depot_id"), $session->get("flux_id"), $session->get('date_distrib'), 0)),
            'comboProduitSup' => $this->getComboHtml($em->getRepository('AmsPaieBundle:PaiPrdTournee')->selectComboSupprimerPresse($session->get("depot_id"), $session->get("flux_id"), $session->get('date_distrib'), 0)),
    ...
    public function ajaxComboAjouterAction(Request $request) {
        $session = $this->get('session');
        $em = $this->getDoctrine()->getManager();
        return $this->ajaxCombo($em->getRepository('AmsPaieBundle:PaiPrdTournee')->selectComboAjouterPresse($session->get("depot_id"), $session->get("flux_id"), $session->get("date_distrib"), 0));
    }  
    public function ajaxComboSupprimerAction(Request $request) {
        $session = $this->get('session');
        $em = $this->getDoctrine()->getManager();
        return $this->ajaxCombo($em->getRepository('AmsPaieBundle:PaiPrdTournee')->selectComboSupprimerPresse($session->get("depot_id"), $session->get("flux_id"), $session->get("date_distrib"), 0));
    }  
}
