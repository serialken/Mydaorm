...
class EmpAffectationController extends GlobalEmployeController {

    public function getRepositoryName() {
        return $this->getBundleName() . ':EmpAffectation';
    }

    public function getRoute() {
        return 'liste_employe_affectation';
    }
    ...
    public function gridAction() {
        ...
        $response = $this->renderView($this->getTwigGrid(), array(
            ...
            'comboEmploye' => $this->getCombo($em->getRepository('AmsEmployeBundle:Employe')->selectComboAffectation($session->get("depot_id"), $session->get("flux_id"), $session->get("anneemois_id"))),
    ...
    public function ajaxActualisationAction(Request $request) {
        $session = $this->get('session');
        $em = $this->getDoctrine()->getManager();
        $em->getRepository('AmsEmployeBundle:EmpAffectation')->actualisation($msg, $msgException, $session->get('UTILISATEUR_ID'), $session->get("depot_id"), $session->get("flux_id"));
